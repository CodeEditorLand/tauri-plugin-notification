...
impl ZbusNotificationHandle {
	pub(crate) fn new(
		...
		ZbusNotificationHandle { id, connection, notification }
	}
	...
	pub async fn close(self) { self.close_fallible().await.unwrap(); }
	...
	pub fn update(&mut self) { self.update_fallible().unwrap(); }
}
...
pub async fn get_capabilities_at_bus(
	bus:NotificationBus,
) -> Result<Vec<String>> {
	let connection = zbus::Connection::session().await?;
...
pub async fn get_server_information_at_bus(
	bus:NotificationBus,
) -> Result<xdg::ServerInformation> {
	let connection = zbus::Connection::session().await?;
...
async fn wait_for_action_signal(
	...
	while let Ok(Some(msg)) =
		zbus::MessageStream::from(connection).try_next().await
	{
