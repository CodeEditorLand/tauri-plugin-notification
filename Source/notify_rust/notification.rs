...
// Returns the name of the current executable, used as a default for
...
///
/// A desktop notification is configured via builder pattern, before it is
/// launched with `show()`.
#[derive(Debug, Clone)]
#[non_exhaustive]
pub struct Notification {
	...
	///
	/// # warning
	/// this does not hold all hints, [`Hint::Custom`] and [`Hint::CustomInt`]
	/// are held elsewhere,
	...
	/// Lifetime of the Notification in ms. Often not respected by server,
	/// sorry.
...
impl Notification {
	...
	/// This is for testing purposes only and will not work with actual
	/// implementations.
	#[cfg(all(unix, not(target_os = "macos")))]
	#[doc(hidden)]
	...
	///
	/// # Platform Support
	/// Please note that this method has no effect on macOS. Here you can only
	...
	/// Set the `subtitle`.
	///
	/// This is only useful on macOS, it's not part of the XDG specification and
	...
	/// Simple html markup should be supported, depending on the server
	/// implementation.
	...
	///
	/// # Platform support
	/// macOS does not have support manually setting the icon. However you can
	/// pretend to be another app using
	...
	///
	/// # Platform support
	/// macOS does not support manually setting the icon. However you can
	/// pretend to be another app using
	/// [`set_application()`](fn.set_application.html)
	...
	///
	/// # Platform support
	/// Most of these hints don't even have an effect on the big XDG Desktops,
	/// they are completely tossed on macOS.
	#[cfg(all(unix, not(target_os = "macos")))]
	...
	/// ## `i31`
	///
	/// This sets the time (in milliseconds) from the time the notification is
	/// displayed until it is closed again by the Notification Server.
	...
	///
	/// When passing a [`Duration`](`std::time::Duration`) we will try convert
	/// it into milliseconds.
	///
	/// # Platform support
	/// This only works on XDG Desktops, macOS does not support manually setting
	/// the timeout.
	...
	///
	/// # Platform support
	/// Most Desktops on linux and bsd are far too relaxed to pay any attention
	/// to this. In macOS this does not exist
	#[cfg(all(unix, not(target_os = "macos")))]
	...
	///
	/// Setting the id ahead of time allows overriding a known other
	/// notification. Though if you want to update a notification, it is easier
	/// to use the `update()` method of the `NotificationHandle` object that
	/// `show()` returns.
	///
	/// (xdg only)
	...
	///
	/// Part of the builder pattern, returns a complete copy of the built
	/// notification.
	pub fn finalize(&self) -> Notification { self.clone() }
	...
	/// This is a raw `f64`, if that is a bit too raw for you please activate
	...
	/// telling the success of the notification.
	#[cfg(target_os = "windows")]
	pub fn show(&self) -> Result<()> { windows::show_notification(self) }
