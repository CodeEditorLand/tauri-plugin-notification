...
impl<R:Runtime> NotificationBuilder<R> {
	...
	/// Make sure the channel exists with {@link listChannels} and {@link
	/// createChannel}.
	...
	/// Detail text for the notification with `largeBody`, `inboxLines` or
	/// `groupSummary`.
	...
	/// Instructs the system that this notification is the summary of a group on
	/// Android.
	...
	/// It is typically used to indicate a background task that is pending (e.g.
	/// a file download) or the user is engaged with (e.g. playing music).
	...
	/// Changes the notification presentation to be silent on iOS (no badge, no
	/// sound, not listed).
...
/// Extensions to [`tauri::App`], [`tauri::AppHandle`],
/// [`tauri::WebviewWindow`], [`tauri::Webview`] and [`tauri::Window`] to access
